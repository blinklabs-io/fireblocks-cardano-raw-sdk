...
export type ChainReadOperation =
  | "address-history"
  | "asset-details"
  | "stake-account"
  | "stake-addresses"
...
export interface ChainPoolMetadata {
  ...
  /** Metadata is untrusted data; the SDK never follows its URLs. */
}
...
export interface ChainQueries {
  ...
  ): Promise<ChainPage<ChainHistoryEntry>>;
  ...
  poolDelegators(
    poolId: string,
    options?: ChainPageOptions
  ): Promise<ChainPage<ChainPoolDelegator>>;
}

export class ChainReadUnsupportedError extends Error {
  constructor(
    public readonly provider: "iagon" | "demeter",
