import { TransactionInput, TransactionHash, BigNum, TransactionOutput, MultiAsset, Assets, Value, ScriptHash, AssetName, TransactionBody, TransactionInputs, TransactionOutputs, LinearFee, min_fee, min_ada_for_output, DataCost, } from "@emurgo/cardano-serialization-lib-nodejs";
import { SupportedAssets, } from "../types/index.js";
...
const addSafeQuantity = (left, right) => {
...
const CARDANO_LINEAR_FEE = LinearFee.new(_feeA, // coefficient (lovelace per byte)
...
export const validateProtocolParameters = (parameters, expectedMagic) => {
    for (const value of Object.values(parameters)) {
        if (!Number.isSafeInteger(value) || value < 0)
            throw new Error("Invalid protocol parameter snapshot");
    }
    if (!parameters.maxTxSize ||
...
const outputMinimum = (output, parameters) => {
    if (!parameters)
        return min_ada_for_output(output, DATA_COST); // Legacy IAGON compatibility.
 ...
 */
export const calculateMinLovelaceForUtxo = (numPolicies) => {
    if (numPolicies === 0) {
 ...
 */
export const countDistinctPolicies = (assets) => {
...
export const getExtraPolicies = (changeAssets, intendedPolicies) => {
 ...
 */
export const calculateTransactionFee = (tx) => {
    const calculatedFee = min_fee(tx, CARDANO_LINEAR_FEE);
    return parseInt(calculatedFee.to_str());
};
export const fetchAndSelectUtxosForCnt = async (params) => {
    const { chainProvider, address, tokenPolicyId, requiredTokenAmount, transactionFee, tokenName, lock = false, } = params;
    try {
        ...
        for (const { utxo, tokenAmount, adaAmount } of tokenUtxosWithAmounts) {
            if (selectedUtxos.length >= CardanoConstants.MAX_TX_INPUTS) {
                logger.warn(`Input limit (${CardanoConstants.MAX_TX_INPUTS}) reached during token UTXO selection - address may be dust-attacked`);
                break;
            }
            selectedUtxos.push(utxo);
            accumulatedTokenAmount += tokenAmount;
            accumulatedAda = addSafeQuantity(accumulatedAda, adaAmount);
            if (accumulatedTokenAmount >= requiredTokenAmount &&
        ...
        logger.info(`Calculated minimums - Recipient: ${actualMinRecipient}, Change: ${actualMinChange} (${numChangePolicies} policies)`);
        ...
        if (accumulatedAda < adaTarget) {
            ...
            for (const { utxo, adaAmount } of adaUtxos) {
                if (selectedUtxos.length >= CardanoConstants.MAX_TX_INPUTS) {
                    logger.warn(`Input limit (${CardanoConstants.MAX_TX_INPUTS}) reached during ADA supplement selection - address may be dust-attacked`);
                    break;
                }
                selectedUtxos.push(utxo);
                accumulatedAda = addSafeQuantity(accumulatedAda, adaAmount);
        ...
        if (selectedUtxos.length >= CardanoConstants.MAX_TX_INPUTS && accumulatedAda < adaTarget) {
            throw new Error(`Input cap (${CardanoConstants.MAX_TX_INPUTS} UTxOs) reached but only ${accumulatedAda} of ${adaTarget} lovelace accumulated. ` +
        ...
        throw new Error(`Error fetching and selecting UTXOs: ${error instanceof Error ? error.message : error}`, { cause: error });
    }
};
export const fetchUtxos = async (chainProvider, address) => {
    try {
        ...
        logger.error(`Error fetching UTXOs for ${address}: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error.stack : undefined);
        return [];
    }
};
export const calculateTokenAmount = (utxo, policyId, tokenName) => {
    ...
    // - Change calculation
    if ((tokenName === SupportedAssets.ADA || tokenName === SupportedAssets.ADA_TEST) &&
        policyId === "") {
...
export const getLovelaceAmount = (utxo) => {
    return utxo.value.lovelace;
};
export const filterUtxos = (utxos, tokenPolicyId, tokenName) => {
    try {
        logger.info(`Filtering ${utxos.length} UTXOs for token: ${tokenPolicyId}.${tokenName}`);
        // Check if this is an ADA transfer
        const isAdaTransfer = (tokenName === SupportedAssets.ADA || tokenName === SupportedAssets.ADA_TEST) &&
            tokenPolicyId === "";
        ...
        if (filtered.length === 0) {
            throw new Error(`No UTXOs found containing token '${tokenName}' with policy ID '${tokenPolicyId}'.`);
        }
        return filtered;
    }
    catch (err) {
        throw new Error(`An unexpected error occurred while filtering UTXOs. ${err instanceof Error ? err.message : String(err)}`, { cause: err });
    }
};
export const createTransactionInputs = (selectedUtxos) => {
...
export const createTransactionOutputs = (params) => {
    const { requiredLovelace, fee, recipientAddress, senderAddress, tokenPolicyId, tokenName, transferAmount, selectedUtxos, } = params;
    ...
    let recipientMultiAsset = null;
    let tempLovelaceBN = null;
    let tempRecipientValue = null;
    let tempRecipientOutput = null;
    let minRecipientBN = null;
    let recipientLovelace;
    try {
        ...
        if (recipientMultiAsset)
            recipientMultiAsset.free();
        if (tempLovelaceBN)
            tempLovelaceBN.free();
        if (tempRecipientValue)
            tempRecipientValue.free();
        if (tempRecipientOutput)
            tempRecipientOutput.free();
        if (minRecipientBN)
            minRecipientBN.free();
        throw e;
    ...
    if (changeLovelace < 0) {
        recipientMultiAsset.free();
        throw new Error(`Insufficient funds: inputs ${totalLovelace} lovelace < recipient ${recipientLovelace} + fee ${fee} = ${recipientLovelace + fee} lovelace`);
    }
    ...
    const changeOutput = buildValidatedOutput(senderAddress, changeLovelace, changeMultiAsset, "change", params.protocolParameters);
 ...
 */
export const buildCntTransactionWithCalculatedFee = (params, txInputs, ttl, estimatedWitnessCount) => {
    return convergeTransactionFee((fee) => createTransactionOutputs({ ...params, fee }), txInputs, ttl, estimatedWitnessCount, "CNT", params.protocolParameters);
};
export const buildTransaction = ({ txInputs, txOutputs, fee, ttl, }) => {
...
export const submitTransaction = async (chainProvider, signedTx) => {
    try {
        ...
        try {
            ...
            if (chainProvider.kind === "demeter" &&
                ...
                if (confirmed?.success && confirmed.data.tx_hash.toLowerCase() === expectedHash)
                    return expectedHash;
                throw new Error(`Submission outcome unknown for ${expectedHash}; check inclusion before retrying`, { cause: error });
            }
            throw error;
        }
        if (response.success) {
            if (response.data.txHash.toLowerCase() !== expectedHash) {
                throw new SdkApiError(`Submission hash mismatch; expected ${expectedHash}`, 502, "SubmissionHashMismatch");
            }
        ...
        if (error instanceof SdkApiError)
            throw error;
        throw new Error(`Error submitting transaction: ${error instanceof Error ? error.message : error}`, { cause: error });
    }
};
/**
 ...
 * Module-private helper - not exported.
 */
const buildMultiAssetFromGrouped = (assetsByPolicy) => {
 ...
 * Module-private helper - not exported.
 */
const buildValidatedOutput = (address, lovelace, multiAsset, label, parameters) => {
    ...
    if (lovelace < minLovelace) {
        output.free();
        throw new Error(`${label} output: insufficient ADA - ${lovelace} lovelace available, ` +
 ...
 */
const convergeTransactionFee = (buildOutputsFn, txInputs, ttl, witnessCount, label, parameters) => {
    if (parameters)
        validateProtocolParameters(parameters);
 ...
 */
export const fetchAndSelectUtxosForAda = async (params) => {
    ...
    for (const utxo of adaOnlyUtxos) {
        if (selectedUtxos.length >= CardanoConstants.MAX_TX_INPUTS) {
            logger.warn(`Input limit (${CardanoConstants.MAX_TX_INPUTS}) reached during ADA-only UTXO selection - address may be dust-attacked`);
            break;
        }
        selectedUtxos.push(utxo);
    ...
    if (accumulatedAda < phase1Target) {
        for (const utxo of multiAssetUtxos) {
            if (selectedUtxos.length >= CardanoConstants.MAX_TX_INPUTS) {
                logger.warn(`Input limit (${CardanoConstants.MAX_TX_INPUTS}) reached during multi-asset UTXO selection - address may be dust-attacked`);
                break;
            }
            selectedUtxos.push(utxo);
            accumulatedAda = addSafeQuantity(accumulatedAda, utxo.value.lovelace);
            ...
            if (accumulatedAda >= lovelaceAmount + transactionFee + tokenChangeMin)
                break;
        }
    }
    ...
    if (selectedUtxos.length >= CardanoConstants.MAX_TX_INPUTS && accumulatedAda < required) {
        throw new Error(`Input cap (${CardanoConstants.MAX_TX_INPUTS} UTxOs) reached but only ${accumulatedAda} of ${required} lovelace accumulated. ` +
            `This address may be dust-attacked with many small UTxOs. Consider consolidating UTxOs first.`);
    }
    logger.info(`ADA UTXO selection: ${selectedUtxos.length} selected (${adaOnlyUtxos.length} ADA-only available), ` +
 ...
 */
export const createAdaTransactionOutputs = (params) => {
    ...
    const recipientOutput = buildValidatedOutput(recipientAddress, lovelaceAmount, null, "ADA recipient", params.protocolParameters);
    // Change output: remaining ADA + ALL tokens from spent UTxOs
    const allTokens = collectAllAssets(selectedUtxos);
    const changeMultiAsset = Object.keys(allTokens).length > 0
        ? buildMultiAssetFromGrouped(groupAssetsByPolicy(allTokens))
        : null;
    const changeOutput = buildValidatedOutput(senderAddress, changeLovelace, changeMultiAsset, "ADA change", params.protocolParameters);
    logger.info(`ADA outputs: recipient ${lovelaceAmount} lovelace, change ${changeLovelace} lovelace` +
 ...
 * @param estimatedWitnessCount - Number of signatures - use WITNESS_COUNT_* constants
 */
export const buildAdaTransactionWithCalculatedFee = (params, txInputs, ttl, estimatedWitnessCount) => {
    return convergeTransactionFee((fee) => createAdaTransactionOutputs({ ...params, fee }), txInputs, ttl, estimatedWitnessCount, "ADA", params.protocolParameters);
};
 ...
 */
export const fetchAndSelectUtxosForMultiToken = async (params) => {
    ...
    const tokenUtxos = utxos
        .filter((u) => u.value.assets && Object.keys(u.value.assets).some((k) => required[k] !== undefined))
        .sort((a, b) => {
        const score = (u) => Object.keys(u.value.assets || {}).filter((k) => required[k] !== undefined).length;
    ...
    for (const utxo of tokenUtxos) {
        if (selectedUtxos.length >= CardanoConstants.MAX_TX_INPUTS) {
            logger.warn(`Input limit (${CardanoConstants.MAX_TX_INPUTS}) reached during multi-token UTXO selection - address may be dust-attacked`);
            break;
        }
        selectedUtxos.push(utxo);
        accumulatedAda = addSafeQuantity(accumulatedAda, utxo.value.lovelace);
        ...
        if (Object.keys(required).every((k) => (accumulated[k] || 0) >= required[k]))
            break;
    }
    // Validate all token requirements are met
    for (const [key, needed] of Object.entries(required)) {
        if ((accumulated[key] || 0) < needed) {
            throw new Error(`Insufficient balance for token ${key}: have ${accumulated[key] || 0}, need ${needed}`);
        }
    }
    ...
    const minRecipient = Math.max(lovelaceAmount ?? 0, calculateMinLovelaceForUtxo(recipientPolicyCount));
    const getMinChange = () => calculateMinLovelaceForUtxo(countDistinctPolicies(collectAllAssets(selectedUtxos)));
    ...
    if (selectedUtxos.length >= CardanoConstants.MAX_TX_INPUTS && accumulatedAda < adaRequired) {
        throw new Error(`Input cap (${CardanoConstants.MAX_TX_INPUTS} UTxOs) reached but only ${accumulatedAda} of ${adaRequired} lovelace accumulated. ` +
            `This address may be dust-attacked with many small UTxOs. Consider consolidating UTxOs first.`);
    }
    logger.info(`Multi-token UTXO selection: ${selectedUtxos.length} selected, ` +
        `${accumulatedAda} lovelace accumulated, ` +
 ...
 * Throws if either output would violate the Cardano min-UTxO rule.
 */
export const createMultiTokenTransactionOutputs = (params) => {
    ...
    const changeMultiAsset = buildMultiAssetFromGrouped(groupAssetsByPolicy(changeTokensFlat));
    const recipientOutput = buildValidatedOutput(recipientAddress, recipientLovelace, recipientMultiAsset, "multi-token recipient", params.protocolParameters);
    recipientMultiAsset.free();
    const changeOutput = buildValidatedOutput(senderAddress, changeLovelace, changeMultiAsset, "change", params.protocolParameters);
    changeMultiAsset?.free();
    logger.info(`Multi-token outputs: recipient gets ${tokens.length} token type(s) with ${recipientLovelace} lovelace, ` +
 ...
 * @param estimatedWitnessCount - Number of signatures - use WITNESS_COUNT_* constants
 */
export const buildMultiTokenTransactionWithCalculatedFee = (params, txInputs, ttl, estimatedWitnessCount) => {
    return convergeTransactionFee((fee) => createMultiTokenTransactionOutputs({ ...params, fee }), txInputs, ttl, estimatedWitnessCount, "multi-token", params.protocolParameters);
};
 ...
 */
export const createConsolidationOutput = (params) => {
    ...
    if (outputLovelace < CardanoConstants.MIN_UTXO_BASE_LOVELACE) {
        throw new Error(`Insufficient ADA for consolidation: fee (${fee} lovelace) leaves only ` +
            `${outputLovelace} lovelace, below the ${CardanoConstants.MIN_UTXO_BASE_LOVELACE} lovelace minimum`);
    }
    const allTokens = collectAllAssets(selectedUtxos);
    const multiAsset = Object.keys(allTokens).length > 0
        ? buildMultiAssetFromGrouped(groupAssetsByPolicy(allTokens))
        : null;
    const output = buildValidatedOutput(senderAddress, outputLovelace, multiAsset, "consolidation", params.protocolParameters);
    logger.info(`Consolidation output: ${outputLovelace} lovelace with ${Object.keys(allTokens).length} token assets`);
    return [output];
};
/**
 ...
 * @param estimatedWitnessCount - Number of signatures - use WITNESS_COUNT_* constants
 */
export const buildConsolidationTransactionWithCalculatedFee = (params, txInputs, ttl, estimatedWitnessCount) => {
    return convergeTransactionFee((fee) => createConsolidationOutput({ ...params, fee }), txInputs, ttl, estimatedWitnessCount, "consolidation", params.protocolParameters);
};
