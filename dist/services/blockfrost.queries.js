...
import { hash, integer, quantity, metadata, parse, parseInput, pageOptions, historyOptions, paymentAddress, stakeAddress, poolId, unitSchema, fingerprint, isoTime, pageResult, } from "./chain-query.validation.js";
const segment = encodeURIComponent;
/** Indexed reads only. No pool validation, certificate creation or signing is implied. */
export class BlockfrostQueries {
    get;
    provider;
    ...
    constructor(get, provider) {
        this.get = get;
        this.provider = provider;
    }
    async read(operation, path, params) {
        try {
            ...
            if (error instanceof SdkApiError)
                throw new SdkApiError(`Demeter ${operation} read failed (HTTP ${error.statusCode ?? "unavailable"})`, error.statusCode, "ChainQueryFailed");
            throw error;
        }
    }
    async addressHistory(address, options = {}) {
        ...
        if (opts.fromBlock !== undefined)
            params.from = opts.fromBlock;
        if (opts.toBlock !== undefined)
            params.to = opts.toBlock;
        const raw = parse(z.array(z.object({ tx_hash: hash, tx_index: integer, block_height: integer, block_time: integer })), await this.read("address-history", `/addresses/${segment(address)}/transactions`, params), "address history");
        for (let i = 0; i < raw.length; i++) {
            if ((opts.fromBlock !== undefined && raw[i].block_height < opts.fromBlock) ||
        ...
        })), opts, (item) => item.txHash, null, "desc");
    ...
    async assetDetails(inputUnit) {
        ...
        const raw = parse(z.object({
            ...
            metadata,
        }), await this.read("asset-details", `/assets/${segment(unit)}`), "asset details");
        if (raw.asset !== unit ||
    ...
    async stakeAccount(input) {
        ...
        }), await this.read("stake-account", `/accounts/${segment(address)}`), "stake account");
    ...
    async stakeAddresses(input, options = {}) {
        ...
        const raw = parse(z.array(z.object({ address: z.string() })), await this.read("stake-addresses", `/accounts/${segment(address)}/addresses`, opts), "stake addresses");
        return pageResult(raw.map((item) => paymentAddress(item.address)), opts, (item) => item);
    }
    async stakeRewards(input, options = {}) {
        const address = stakeAddress(input);
        const opts = parseInput(pageOptions, options, "page options");
        const raw = parse(z.array(z.object({ epoch: integer, amount: quantity, pool_id: z.string(), type: z.string().min(1) })), await this.read("stake-rewards", `/accounts/${segment(address)}/rewards`, opts), "stake rewards");
    ...
    async poolMetadata(input) {
        ...
        }), await this.read("pool-metadata", `/pools/${segment(id)}/metadata`), "pool metadata");
    ...
    async poolDelegators(input, options = {}) {
        const id = poolId(input);
        const opts = parseInput(pageOptions, options, "page options");
        const raw = parse(z.array(z.object({ address: z.string(), live_stake: quantity })), await this.read("pool-delegators", `/pools/${segment(id)}/delegators`, opts), "pool delegators");
        return pageResult(raw.map((item) => ({
            stakeAddress: stakeAddress(item.address),
            amount: item.live_stake,
            amountKind: "live-stake",
            activeEpoch: null,
        })), opts, (item) => item.stakeAddress);
    }
}
//# sourceMappingURL=blockfrost.queries.js.map